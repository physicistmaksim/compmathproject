...
def snap_collision_velocities(a: np.ndarray, xi_grid: tuple, dxi: tuple) -> np.ndarray:
    """
    3) «Привязывает» скорости столкновения, сгенерированные рандомно, к ближайшим узлам скоростной сетки.
    ...
    for i, grid in enumerate((xi_x, xi_y, xi_z, xi_x, xi_y, xi_z)): #grid — сетка узлов, соответствующая текущей компоненте
        # индекс ближайшего узла
        idx = np.round((a[i] - grid[0]) / dxi[i % 3]).astype(int) #вычисляем, на сколько шагов каждая скорость a[i] отстоит от начала сетки.
...
def filter_collision_nodes(a: np.ndarray, snapped_idx : np.ndarray, xi_cut: float) -> np.ndarray:
    """
...
def compute_post_collision_velocities_sem_2(a: np.ndarray, d: float) -> np.ndarray:
    ...
    # 6. Расчет скоростей после столкновения (формула 1.8)
...
def find_interpolating_nodes_and_weights(a_updated: np.ndarray, xi_grid: tuple, dxi: tuple, xi_cut: float):
    """
    6) находим λ_ν, λ_ν + s_ν - координаты приближающих узлов для разлетной скорости ξ', μ_ν, μ_ν - s_ν - координаты приближающих узлов для скорости ξ₁', а также r_ν
    Параметры:
    updated_a : np.ndarray, shape (8, m)
        Обновлённые узлы после столкновенияы: первые 6 строк — ξ', ξ₁', 7-я — b, 8-я — ε.
    ...
    """
    #A) ищем ближайшие узлы для  xi_p и xi1_p и отсевиваем те которые не попдают в сферу радиуса xi_cut
    ...
    eta_near = snapped_idx_updated_filtered[0:3, :] #массив ближайших точек из узлов сетки, shape(3, m) для скорости xi
    velocity_near = a_updated_snapped_filtered[0:3, :] #массив ближайших координат из узлов сетки, shape(3, m) для скорости xi
    eta_near1 = snapped_idx_updated_filtered[3:6, :] #то же для скорсоти xi1
    velocity_near1 = a_updated_snapped_filtered[3:6, :] ##то же для скорсоти xi1

    #Б) ищем 8 узлов, которые окружают точную разлетную скорость для каждого столкнвения
    ...
    shifts_surround = np.zeros((8, 3, m), dtype=int) #8 окружающих точек, для каждой 3 координаты скорости относительно eta_center: i, j, k:
                                                                                                                                    #idx:  i j k
                                                                                                                                    #0     0 0 0
                                                                                                                                    #1     0 0 1
                                                                                                                                    #2     0 1 0
    ...
    nodes_surround = np.zeros((8, 3, m), dtype=int) #8 окружающих точек, для каждой 3 абсолютных координаты скорости (не относительно eta_center)
    nodes_surround1 = np.zeros((8, 3, m), dtype=int) #8 окружающих точек, симметричных тем что в nodes_surround, для каждой 3 абсолютных координаты скорости (не относительно eta_center)
    ...
    velocity_surround = np.zeros((8, 3, m), dtype=float) #тот же массив, но в нем реальные проекции скоростей, а не целые числа
    ...
    for v in range(8):         # 8 вершин
        for dim in range(3):   # 0→x,1→y,2→z
            ...
            idxs = nodes_surround[v, dim, :] # индексы вдоль этой оси для всех m столкновений
            velocity_surround[v, dim, :] = grid[idxs]
    for v in range(8):         # 8 вершин
        for dim in range(3):   # 0→x,1→y,2→z
            grid = xi_grid[dim]              # выбираем нужную одномерную сетку
            idxs = nodes_surround1[v, dim, :] # индексы вдоль этой оси для всех m столкновений
            velocity_surround1[v, dim, :] = grid[idxs]    

    e_near = eta_near - eta_center #координата ближайшего узла относительно центра


    #B) cоставляются кинетические энергии этих узлов.
    ...
    for v in range(8):
        E_surround[v, :] = (velocity_surround[v, 0, :] - xi_c[0, :])**2 + (velocity_surround[v, 1, :] - xi_c[1, :])**2 + (velocity_surround[v, 2, :] - xi_c[2, :])**2
    

    #Г) находим E_near - энергию ближайшего узла
    E_near = (velocity_near[0, :] - xi_c[0, :])**2 + (velocity_near[1, :] - xi_c[1, :])**2 + (velocity_near[2, :] - xi_c[2, :])**2 #энергия ближайшего узла
    E_0 = (a_updated_filtered[0, :] - xi_c[0, :])**2 + (a_updated_filtered[1, :] - xi_c[1, :])**2 + (a_updated_filtered[2, :] - xi_c[2, :])**2 #энергия точной разлетной скорости


    #Д) В соответствии с (2.8) для случая 2 нужно найти такую выборку из соседних узлов, для которых верно Ei,j,k < E0 
    ...
    for i in range(m):
        ...
        if np.isclose(E_0[i], E_near[i]):
            ...
            candidates_velocity = velocity_surround[candidates, :, i] #массив (число подходящих точек, 3) - координаты подходящих точек
            candidates_velocity1 = velocity_surround1[candidates, :, i]
            candidate_nodes_velocity = nodes_surround[candidates, :, i] # #массив (число подходящих точек, 3) - положение на сетке подходящих точек
            ...
            valid_mask = (np.linalg.norm(candidates_velocity, axis=1) <= xi_cut) & (np.linalg.norm(candidates_velocity1, axis=1) <= xi_cut)
            ...
            E2 = E_near[i]
            E1 = (candidates_velocity_filtered[chosen_idx, 0] - xi_c[0, i])**2 + (candidates_velocity_filtered[chosen_idx, 1] - xi_c[1, i])**2 + (candidates_velocity_filtered[chosen_idx, 2] - xi_c[2, i])**2
            r[i] = (E_0[i] - E1) / (E2 - E1)
            ...
            # Коэффициент r из условия (2.4)
            E1 = E_near[i]
            E2= (candidates_velocity_filtered[chosen_idx, 0] - xi_c[0, i])**2 + (candidates_velocity_filtered[chosen_idx, 1] - xi_c[1, i])**2 + (candidates_velocity_filtered[chosen_idx, 2] - xi_c[2, i])**2
            r[i] = (E_0[i] - E1) / (E2 - E1)
    lam1 = lam1[:, keep]
    lam2 = lam2[:, keep]
    mu1 = mu1[:, keep]
